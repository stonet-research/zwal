  ...
  if (s.ok()) {
    ...
    while ((!resuming_from_bg_err || error_handler_.GetRecoveryError().ok())) {
      ...
      if (!res.first.ok()) {
        ...
        // printf("Before wait atomic flush\n");
      atomic_flush_install_cv_.Wait();
        // printf("After wait atomic flush\n");
...
Status DBImpl::PauseBackgroundWork() {
  ...
  while (bg_bottom_compaction_scheduled_ > 0 || bg_compaction_scheduled_ > 0 ||
    ...
    bg_cv_.Wait();
      // fprintf(stdout, "Resume operations after PAUSE in flush\n");
  }
  bg_work_paused_++;
  return Status::OK();
}
...
Status DBImpl::RunManualCompaction(
  ...
  if (exclusive) {
    ...
    while (bg_bottom_compaction_scheduled_ > 0 ||
      ...
      ROCKS_LOG_INFO(
          ...
          cfd->GetName().c_str());
      bg_cv_.Wait();
                    // fprintf(stdout, "Resume operations after run manual compaction\n");
    }
  }
  ...
  while (!manual.done) {
    ...
    if (ShouldntRunManualCompaction(&manual) || (manual.in_progress == true) ||
      ...
      if (!manual.done) {
                              // fprintf(stdout, "Resume operations before wait until flush wouldvfptgvfpgvfpgv\n");
        bg_cv_.Wait();
                      // fprintf(stdout, "Resume operations after wait until flush wouldvfptgvfpgvfpgv\n");
      }
    ...
    do {
      if (write_stall_condition != WriteStallCondition::kNormal) {
        ...
        ROCKS_LOG_INFO(immutable_db_options_.info_log,
                       ...
                       cfd->GetName().c_str());
              // fprintf(stdout, "Resume operations before wait until flush would not stall writes\n");
        bg_cv_.Wait();
              // fprintf(stdout, "Resume operations after wait until flush would not stall writes\n");
      }
      if (cfd->IsDropped()) {
  ...
  while (resuming_from_bg_err || !error_handler_.IsDBStopped()) {
    ...
    if (num_dropped + num_finished == num) {
      break;
                  ...
                  // fprintf(stdout, "Wait before flush memtables\n");
    bg_cv_.Wait();
                  // fprintf(stdout, "Wait for flush memtables\n");
  }
...
void DBImpl::DisableManualCompaction() {
  ...
  while (HasPendingManualCompaction()) {
    bg_cv_.Wait();
                  // fprintf(stdout, "Resume operations after manual compaction\n");
  }
}
...
void DBImpl::BackgroundCallFlush(Env::Priority thread_pri) {
  ...
  TEST_SYNC_POINT("DBImpl::BackgroundCallFlush:Start:2");
  {
    ...
    if (job_context.HaveSomethingToClean() ||
      ...
      log_buffer.FlushBufferToLog();
      if (job_context.HaveSomethingToDelete()) {
                // fprintf(stdout, "Background flush\n");
        PurgeObsoleteFiles(job_context);
                        // fprintf(stdout, "Background flush 2\n");
      }
      job_context.Clean();
      mutex_.Lock();
    }
...
void DBImpl::BackgroundCallCompaction(PrepickedCompaction* prepicked_compaction,
                       ...
                       immutable_db_options_.info_log.get());
  {
    ...
    if (job_context.HaveSomethingToClean() ||
      ...
      log_buffer.FlushBufferToLog();
      if (job_context.HaveSomethingToDelete()) {
                        // fprintf(stdout, "Background call compaction\n");
        PurgeObsoleteFiles(job_context);
                                // fprintf(stdout, "Background call compaction\n");
...
Status DBImpl::WaitForCompact(bool abort_on_pause) {
  ...
  for (;;) {
    ...
    if ((bg_bottom_compaction_scheduled_ || bg_compaction_scheduled_ ||
        ...
        (error_handler_.GetBGError().ok())) {
      bg_cv_.Wait();
                    // fprintf(stdout, "Resume operations after wait for compact\n");
    } else {
      return error_handler_.GetBGError();
    }
  }
