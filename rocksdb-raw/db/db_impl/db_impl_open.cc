...
Status DBImpl::RecoverLogFiles(const std::vector<uint64_t>& wal_numbers,
  ...
  for (auto wal_number : wal_numbers) {
    ...
    {
      FileOptions opts = fs_->OptimizeForLogRead(file_options_);
      opts.use_direct_reads = true;
      std::unique_ptr<FSSequentialFile> file;
      status = fs_->NewSequentialFile(
          fname, opts, &file, nullptr);
      if (!status.ok()) {
        MaybeIgnoreError(&status);
        if (!status.ok()) {
          return status;
    ...
    while (!stop_replay_by_wal_filter &&
      ...
      clock_gettime(CLOCK_MONOTONIC, &tp_4);  
      if (tp_4.tv_nsec < tp_3.tv_nsec) {
        apply_time += 1000'000'000 - (tp_3.tv_nsec - tp_4.tv_nsec);
        tp_4.tv_sec -= 1; 
      } else {
        apply_time += tp_4.tv_nsec - tp_3.tv_nsec;
      }
      apply_time += 1000'000'000 * (tp_4.tv_sec - tp_3.tv_sec);
    ...
    printf("Recovery time %lu, apply time %lu\n", time_passed, apply_time);   
  ...
  if (stop_replay_for_corruption == true &&
    ...
    for (auto cfd : *versions_->GetColumnFamilySet()) {
      ...
      if (cfd->GetLogNumber() > corrupted_wal_number &&
        ...
        printf("Failed recovering WAL (corruption) \n");
  ...
  if (s.ok()) {
    ...
    if (s.ok()) {
      ...
      if (recovered_seq != kMaxSequenceNumber) {
        ...
        printf("s = WAL stuff (%d)\n", s.ok()); fflush(stdout);
      }
    }
  }
  if (s.ok()) {
    s = impl->LogAndApplyForRecovery(recovery_ctx);
    printf("s = LogAndApply (%d)\n", s.ok()); fflush(stdout);
  }

  if (s.ok() && impl->immutable_db_options_.persist_stats_to_disk) {
    impl->mutex_.AssertHeld();
    s = impl->InitPersistStatsColumnFamily();
    printf("s = InitPersists (%d)\n", s.ok()); fflush(stdout);
  }
    ...
    for (auto cf : column_families) {
      ...
      if (cfd != nullptr) {
        ...
        if (db_options.create_missing_column_families) {
          ...
          printf("s = CreateColumnFamilies (%d)\n", s.ok()); fflush(stdout);
          impl->mutex_.Lock();
          if (s.ok()) {
