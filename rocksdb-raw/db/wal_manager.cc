...
void WalManager::PurgeObsoleteWALFiles() {
  ...
  if (!s.ok()) {
    ROCKS_LOG_ERROR(db_options_.info_log, "Can't get current time: %s",
                    s.ToString().c_str());
    assert(false);
    // printf("Purging WAL get time failed\n");
    return;
  }
  ...
  // printf("TIME TO CHECK %lu \n", time_to_check);

  if (purge_wal_files_last_run_ + time_to_check > now_seconds) {
    // printf("Purging WAL time no time left %lu > %lu\n", 
  ...
  if (!s.ok()) {
    ROCKS_LOG_ERROR(db_options_.info_log, "Can't get archive files: %s",
                    s.ToString().c_str());
    assert(false);
    // printf("Purging WAL time children failed\n");
    return;
  }
    ...
    if (ParseFileName(f, &number, &type) && type == kWalFile) {
      ...
      if (ttl_enabled) {
        ...
        if (
          ...
          // printf("Purging WAL DELETE on time\n");
          ...
          continue;
              ...
              printf("Purging WAL time no time left AGAIN %lu > %lu\n", 
                now_seconds - file_m_time, db_options_.WAL_ttl_seconds);
        }
      }
      ...
      if (size_limit_enabled) {
        ...
        if (!s.ok()) {
          ROCKS_LOG_ERROR(db_options_.info_log,
                          ...
                          s.ToString().c_str());
        // printf("Purging WAL time size failed\n");
          return;
        } else {
          if (file_size > 0) {
          ...
          // printf("Purging WAL DELETE on size\n");
            s = DeleteDBFile(&db_options_, file_path, archival_dir, false,
                             /*force_fg=*/!wal_in_db_path_);
            if (!s.ok()) {
              ROCKS_LOG_WARN(db_options_.info_log,
