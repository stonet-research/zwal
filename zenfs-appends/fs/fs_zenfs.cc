...
IOStatus ZenFS::DeleteFileNoLock(std::string fname, const IOOptions& options,
  ...
  if (zoneFile != nullptr) {
    ...
    if (!s.ok()) {
      ...
      // APPEND-DOC, deletion record is persisted to metadata (so resetting zones should be safe)
      // We delete immediately
      if (ends_with(fname, ".log")) {
        zoneFile->ResetWALZones();
      }
...
IOStatus ZenFS::OpenWritableFile(const std::string& filename,
  ...
  // APPEND-DOC, if the file is a WAL, set it here immediately
...
Status ZenFS::Mount(bool readonly) {
  ...
  printf("Booting from modified ZenFS\n");
...
IOStatus ZenFS::MigrateFileExtents(
  ...
  for (ZoneExtent* ext : new_extent_list) {
    ...
    if (zfile->IsSparse()) {
      ...
      // APPEND-DOC, for WALs we have an additional header field
      ...
      zfile->MigrateData(ext->start_ - header_size,
                         ext->length_ + header_size,
                         target_zone);
