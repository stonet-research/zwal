        ...
        for workload in a b c d e; do
            ...
            sudo ./bin/ycsb load rocksdb \
                ...
                -p fieldcount=${fieldcount} \
                    1> ../data/ycsb/${devstr}-${workload}-load-${barrier}-${depth}-${bs}-${run}.out \
                    2> ../data/ycsb/${devstr}-${workload}-load-${barrier}-${depth}-${bs}-${run}.err;
            ...
            sudo ./bin/ycsb run rocksdb \
                ...
                -p fieldlength=${fieldlength} \
                -p fieldcount=${fieldcount} \
                    1> ../data/ycsb/${devstr}-${workload}-run-${barrier}-${depth}-${bs}-${run}.out \
                    2> ../data/ycsb/${devstr}-${workload}-run-${barrier}-${depth}-${bs}-${run}.err;
            popd
        done
    done
done
